...
static db_err_t _facedb_init(void)
{
    ...
    if (db->is_init)    return DB_ERR_REINIT;
    /* Init lock */
    // ...
...
static db_err_t _facedb_deinit(void)
{
    db_err_t err = DB_OK;
    db_t *db = (db_t *)&facedb;
    if (!db->is_init)    return DB_ERR_UNINIT;
...
static db_err_t _facedb_insert(uint8_t *uid, void *item)
{
    ...
    if (!db->is_init)    return DB_ERR_UNINIT;
    if (!uid || !item)  return DB_ERR_PARAM;
    /* Lock */
    if (db->lock)
        db->lock();
...
static int _facedb_iterate(uint8_t *uid, void *item)
{
    ...
    facedb_private_t *private = (facedb_private_t *)db->private;
    if (!db->is_init)    return DB_ERR_UNINIT;
    if (!private->iterate_run)  return DB_ERR_UNINIT;
